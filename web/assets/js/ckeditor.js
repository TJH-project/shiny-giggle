...
(function () {
			...
			on: function (b, d, g, h, l) {
				...
				if (0 > f.getListenerIndex(d)) {
					...
					for (var u = f.length - 1; 0 <= u; u--)
						if (f[u].priority <= l) return f.splice(u + 1, 0, m), {
							removeListener: r
						};
					f.unshift(m)
				}
		...
		CKEDITOR.tools = {
			...
			parseCssText: function (a, b, c) {
				...
				a.replace(/&quot;/g, '"').replace(/\s*([^:;\s]+)\s*:\s*([^;]+)\s*(?=;|$)/g,
					function (a, c, q) {
						b && (c = c.toLowerCase(), "font-family" == c && (q = q.replace(/\s*,\s*/g, ",")), q = CKEDITOR.tools.trim(q));
						l[c] = q
					});
				return l
			},
		...
		CKEDITOR.tools.extend(CKEDITOR.dom.element.prototype, {
			...
			getDocumentPosition: function (a) {
				...
				if (document.documentElement.getBoundingClientRect && (CKEDITOR.env.ie ?
					...
					h && (CKEDITOR.env.webkit || CKEDITOR.env.ie && 12 <= CKEDITOR.env.version ? (b = d.$.scrollLeft || g.scrollLeft, c = d.$.scrollTop || g.scrollTop) : (c = u ? d.$ : g, b = c.scrollLeft, c = c.scrollTop), b = e.left + b - x, c = e.top + c - q)
				} else
			...
			if (k && n.type == CKEDITOR.NODE_TEXT && p.equals(n)) p = a.document.createText(p.substring(t, z)), c.append(p);
				...
				for (S = G; S <= I; S++)
					if (c = v[S], X = c.getPrevious(), c.equals(P[S])) b && (K = K.getChild(0));
						...
						S != I || c.equals(P[S]) && I < D ? b && (M = K.append(c.clone(0, e))) : F ? q(c, K, !1, H) : Q && K.append(a.document.createText(c.substring(0, z)));
						if (S > fa)
							for (; X;) X = q(X, K, !0);
						K = M
					}
		...
		CKEDITOR.dom.range.prototype = {
			...
			enlarge: function (a, b) {
				...
				case CKEDITOR.ENLARGE_LIST_ITEM_CONTENTS:
					...
					var m, Q, P = CKEDITOR.dom.walker.blockBoundary(a == CKEDITOR.ENLARGE_LIST_ITEM_CONTENTS ? {
							br: 1
						} : null),
						v = null,
						G = function (a) {
			...
			getNextParagraph: function (a) {
				...
				for (this._.nextNode = null; x;) {
					...
					if (E) x.type == CKEDITOR.NODE_TEXT && e.test(x.getText()) && (E = 0);
						...
						var p = x.getName();
						if (CKEDITOR.dtd.$block[p] && "false" == x.getAttribute("contenteditable")) {
							d = x;
							b(this, a, d);
							break
							...
							if ("br" == p) E = 1;
							else if (!h && !x.getChildCount() && "hr" != p) {
								d = x;
								u = x.equals(q);
								break
							}
							...
							if (x.getFirst()) {
								h || (h = this.range.clone(), h.setStartAt(x, CKEDITOR.POSITION_BEFORE_START));
								x = x.getFirst();
								continue
							}
							E = 1
	...
	(function () {
		...
		function f(a) {
			var b, c, d, k, e = {},
				p = 1;
			for (a = H(a); b = a.match(v);)(c = b[2]) ? (d = C(c, "styles"), k = C(c, "attrs"), c = C(c, "classes")) : d = k = c = null, e["$" + p++] = {
				elements: b[1],
				classes: c,
				styles: d,
				attributes: k
		...
		function A(a, b, k, e) {
			...
			if (e.doFilter) {
				...
				if (!a.allClasses ||
					...
					v && h.length < v.split(/\s+/).length && (z = !0)
				} else v && (t["class"] = v);z && (n = 1);
		...
		function k(a, b, c) {
			var d = a.name;
			if (F.$empty[d] || !a.children.length) "hr" == d && "br" == b ? a.replaceWith(w()) : (a.parent && c.push({
				check: "it",
				el: a.parent
			}), a.remove());
				...
				if ("br" == b) a.previous && !E(a.previous) && (b = w(), b.insertBefore(a)), a.next && !E(a.next) && (b = w(), b.insertAfter(a)), a.replaceWithChildren();
					...
					if (k) a.name = b, a.attributes = {}, c.push({
						...
						k = a.parent;
						for (var e = k.type == CKEDITOR.NODE_DOCUMENT_FRAGMENT || "body" == k.name, t, f, p = d.length; 0 < p;) n = d[--p], e && (n.type == CKEDITOR.NODE_TEXT || n.type == CKEDITOR.NODE_ELEMENT && F.$inline[n.name]) ? (t || (t = new CKEDITOR.htmlParser.element(b), t.insertAfter(a), c.push({
							check: "parent-down",
							el: t
						})), t.add(n, 0)) : (t = null, f = F[k.name] || F.span, n.insertAfter(a), k.type == CKEDITOR.NODE_DOCUMENT_FRAGMENT || n.type != CKEDITOR.NODE_ELEMENT || f[n.name] || c.push({
							check: "el-up",
							el: n
						}));
						a.remove()
		...
		CKEDITOR.filter.prototype = {
				...
				a.forEach(function (a) {
					if (a.type == CKEDITOR.NODE_ELEMENT) {
						...
						a: {
							...
							1 == d.children.length && (k = d.children[0]).type == CKEDITOR.NODE_ELEMENT && A(e, k, c, g);
							c = !c.length
						}
						c || p.push(a)
					}
				...
				if (!this.disabled && a) {
					...
					for (k = 0; k < a.length; ++k) {
						...
						for (p = 0; p < b.length; ++p) n = b[p], "string" == typeof n ? (n = n.split(/\s*:\s*/), t = n[0], f = null, g = n[1]) : (t = n.check, f = n.left, g = n.right), e || (e = n, e = e.element ? e.element : t ? t.match(/^([a-z0-9]+)/i)[0] :
							e.left.getDefinition().element), f instanceof CKEDITOR.style && (f = z(f)), c.push({
							check: t == e ? null : t,
							left: f,
							right: "string" == typeof g ? B(g) : g
						});
						b = e;
						d[b] || (d[b] = []);
						d[b].push(c)
			...
			check: function (a, b, c) {
				...
				if ("string" == typeof a) {
					...
					k = d
				} else d = a.getDefinition(), k = d.styles, n = d.attributes || {}, k && !CKEDITOR.tools.isEmpty(k) ? (k = L(k), n.style = CKEDITOR.tools.writeCssText(k, !0)) : k = {}, k = {
					name: d.element,
					attributes: n,
		...
		function C(a) {
			...
			CKEDITOR.plugins.load(c.split(","), function (c) {
				...
				for (var f in c) {
					...
					if (m && (r = m.match(p)))
						for (; m = r.pop();) CKEDITOR.error("editor-plugin-required", {
							plugin: m.replace(",", ""),
							requiredBy: f
						});
		...
		CKEDITOR.editor.prototype.setMode = function (a, b) {
			...
			if (a != d.mode && h && h[a]) {
				...
				if (d.mode) {
					...
					"source" == d.mode && h == m && (d.fire("lockSnapshot", {
						forceUpdate: !0
					}), r = 1);
				...
				setup: function () {
					...
					if (!this.is("textarea")) {
						...
						a.blockless && CKEDITOR.env.ie && CKEDITOR.env.needsBrFiller && this.attachListener(this, "keyup", function (b) {
							b.data.getKeystroke() in F && !this.getFirst(c) && (this.appendBogus(), b = a.createRange(), b.moveToPosition(this, CKEDITOR.POSITION_AFTER_START), b.select())
						});
		...
		q = function () {
			...
			function b(c, d) {
				...
				for (var h = q[h.getName()], l = 0, B = c.getChildren(), z = B.count(), F = -1, m = -1, r = 0, H = e.contains(q.$list); l < z; ++l) e = B.getItem(l), a(e) ? (n = e.getName(), H && n in CKEDITOR.dtd.$list ? g = g.concat(b(e, d)) : (t = !!h[n],
					"br" != n || !e.data("cke-eol") || l && l != z - 1 || (r = (f = l ? g[l - 1].node : B.getItem(l + 1)) && (!a(f) || !f.is("br")), f = f && a(f) && q.$block[f.getName()]), -1 != F || t || (F = l), t || (m = l), g.push({
						isElement: 1,
						isLineBreak: r,
					...
					node: e,
					allowed: 1
				}); - 1 < F && (g[F].firstNotAllowed = 1); - 1 < m && (g[m].lastNotAllowed = 1);
				return g
			}
					...
					if (H) {
						...
						for (F(v, w); D < v.length; D++) {
							...
							if (y = M.isLineBreak) {
								...
								var U = void 0,
									aa = void 0;
								M.hasBlockSibling ? y = 1 : (U = y.startContainer.getAscendant(q.$block, 1)) && U.is({
									div: 1,
									p: 1
								}) ? (aa = U.getPosition(E), aa == CKEDITOR.POSITION_IDENTICAL || aa == CKEDITOR.POSITION_CONTAINS ? y = 0 : (E = y.splitElement(U), y.moveToPosition(E, CKEDITOR.POSITION_AFTER_START), y = 1)) : y = 0
							}
							if (y) G = 0 < D;
							else {
								y = w.startPath();
	...
	(function () {
		...
		CKEDITOR.on("instanceCreated", function (b) {
			...
			e.on("contentDom", function () {
				...
				if (CKEDITOR.env.ie || m) C ? g.attachListener(g, "beforedeactivate", b, null, null, -1) : g.attachListener(e, "selectionCheck", b, null, null, -1), g.attachListener(g, CKEDITOR.env.webkit ? "DOMFocusOut" : "blur", function () {
					e.lockSelection(v);
					q = 1
				...
				if (a.length)
					if (this.isLocked) {
						...
						a: {
							var m, q;
							if (1 == a.length &&
								!(q = a[0]).collapsed && (l = q.getEnclosedNode()) && l.type == CKEDITOR.NODE_ELEMENT && (q = q.clone(), q.shrink(CKEDITOR.SHRINK_ELEMENT, !0), (m = q.getEnclosedNode()) && m.type == CKEDITOR.NODE_ELEMENT && (l = m), "false" == l.getAttribute("contenteditable"))) break a;
							l = void 0
						}
						if (l) this.fake(l);
						else {
							if (C) {
								...
								if ((c = l.getEnclosedNode()) && c.type == CKEDITOR.NODE_ELEMENT &&
									c.getName() in E && (!c.is("a") || !c.getText())) try {
								...
								for (y = 0; y < a.length; y++) {
									if (y < a.length - 1 && (r = a[y], u = a[y + 1], m = r.clone(), m.setStart(r.endContainer, r.endOffset), m.setEnd(u.startContainer, u.startOffset), !m.collapsed && (m.shrink(CKEDITOR.NODE_ELEMENT, !0), l = m.getCommonAncestor(), m = m.getEnclosedNode(), l.isReadOnly() || m && m.isReadOnly()))) {
